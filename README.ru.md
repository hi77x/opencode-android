...
- **Первый запуск**: разрешите доступ к файлам (сессии хранятся в `/sdcard/OpenCode` и переживают переустановку), затем Настройки → Провайдеры → подключите провайдера и добавьте проект (`~/workspace`)
